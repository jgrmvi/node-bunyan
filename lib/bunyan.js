...
function Logger(options, _childOptions, _childSimple) {
    ...
    if (options.namespace && typeof (options.namespace) !== 'object') {
        throw new TypeError('invalid options.namespace: must be an object');
    }
    if (options.namespace && !options.namespace.name) {
        throw new TypeError('invalid options.namespace.name: must be a string');
    }
    if (options.namespace && (!options.namespace.attributes || !Array.isArray(options.namespace.attributes))) {
        throw new TypeError('invalid options.namespace.name: must be a string');
    }
    ...
    if (parent && _childSimple) {
        ...
        this.namespaceAttributes = parent.namespaceAttributes;
    ...
    if (parent) {
        ...
        this.src = parent.src;
        this.namespace = parent.namespace;
        this.namespaceAttributes = objCopy(parent.namespaceAttributes);
    ...
    if (options.namespace) {
        this.namespace = require('continuation-local-storage').getNamespace(options.namespace.name);
        this.namespaceAttributes = options.namespace.attributes;
    }
    xxx('Logger: ', self)
    ...
    return function () {
        ...
        function mkRecord(args) {
            ...
            if (args[0] instanceof Error) {
                ...
                if (log.namespace) {
                    log.namespaceAttributes.forEach(function (attribute) {
                        var attributeValue = log.namespace.get(attribute);
                        if (attributeValue) {
                            args[0][attribute] = attributeValue;
                        }
                    });
                }
                fields = args[0];
